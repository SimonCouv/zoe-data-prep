...
a <- fread(sprintf("%s/linked_cleaned_twins_assessments_export_%s.csv", params$wdir, params$timestamp))
timestamp_date <- as_date(substr(timestamp, 1, 8))

ct <- fread(sprintf("%s/linked_twins_covid_test_export_%s.csv", params$wdir, params$timestamp))
...
Individuals in TUK with >= 1 assessment (after data cleaning): `r n_distinct(a$patient_id)`
...
total_with_test <- ct %>% 
  dplyr::filter(nchar(result) > 0) %>%
  {n_distinct(.$TwinSN)}

total_with_test_result <- ct %>% 
  dplyr::filter(result %in% c("positive", "negative")) %>% 
  {n_distinct(.$TwinSN)}

total_with_positive_test <- ct %>% 
  dplyr::filter(result == "positive") %>%
  {n_distinct(.$TwinSN)}

```

Total tested individuals: `r total_with_test`


Of those, total with a result available: `r total_with_test_result`


Of those, total with a positive result: `r total_with_positive_test`
...
a %>% 
  ...
  ungroup() %>% 
  count(date_updated_at, date_first_log, .drop=F) %>%
  complete(
    date_updated_at = seq(min(date_updated_at, na.rm = T),timestamp_date, by="day"),
    date_first_log = seq(min(date_first_log, na.rm = T),timestamp_date, by="day"),
    fill = list(n=0)
  ) %>% 
  dplyr::filter(date_updated_at >= date_first_log & date_updated_at < timestamp_date) %>% 
...
retention %>% 
  ...
  geom_tile()+
  geom_text(aes(label=round(retention)), size=1)+
  theme_bw()+
  ylab("date of first report")+
  xlab("report day")+
